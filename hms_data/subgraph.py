...
async def query_subgraph(
    ...
    headers = {
        ...
        "Authorization": "Bearer 026d1a7c8978310b3c41192f862bb394"
    }
    try:
...
async def get_license(
...
) -> Tuple[Optional[ProposalData], bool]:
    # Create cache directory if it doesn't exist
    Path(cache_dir).mkdir(exist_ok=True)
    ...
    async with ClientSession() as session:
        res = await query_subgraph(
            session,
            # SUBGRAPHS["mainnet"]["ethereum"],
            "https://gateway.thegraph.com/api/subgraphs/id/6kPstUcYXBVsJ6iQt5emYCSKiAjpA759RkxyYYhgXav2",
            query,
        )
